class AttendingsController < ApplicationController
  ...
  def create
    if Attending.find_by(attending_params).nil?
      ...
      respond_to do |format|
        if @attending.save
          format.html { redirect_to my_events_path, notice: "You are regestring this event!" }
        else
          format.html { redirect_to my_events_path, alert: "An error occured." }
        end
      end
    else
      flash.alert = 'You are already regestring.'
  ...
  def destroy
    ...
    if @attending.nil?
      flash.alert = "This event register does not exist"
    elsif @attending.destroy
      flash.notice = "You are no longer regestring this event."
    else
      flash.alert = "Error - register was not deleted."
    end
    redirect_to my_events_path
  end
